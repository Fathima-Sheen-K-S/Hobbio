...
if(isset($_POST["btnchangepass"]))
{
	...
	$selpassqry="select *from tbl_center where center_id=".$centerid;
	$result=$con->query($selpassqry);
	$row=$result->fetch_assoc();
	$oldpass1=$row['center_password'];
	...
	if($oldpass==$oldpass1)
	{
		...
		{
			$updqry="update tbl_center set center_password='".$newpass."' where center_id=".$centerid;
			if($con->query($updqry))
			{
				?>
               <script>
  document.addEventListener("DOMContentLoaded", function() {
    Swal.fire({
      ...
      window.location = "CenterHome.php";
    });
  });
</script>
                    <?php
  ...
  document.addEventListener("DOMContentLoaded", function() {
    Swal.fire({
      title: "Re-enter the New Password Correctly",
      icon: "error"
    }).then(() => {
      window.location = "CenterChangePassword.php";
    });
  });
</script>
...
<title>Hobbio::Center Change Password</title>
</head>
...
<?php
$selqry="select tbl_center.center_password from tbl_center";
$res=$con->query($selqry);
$row=$res->fetch_assoc();
$upass=$row['center_password'];
...
<table style=" width: 500px;
  ...
  <tr>
    ...
    border-radius: 4px;" required placeholder="Enter Old Password"/>
    <i id="togglePassword" class="fas fa-eye" style="position: relative;
    top: -50px;
    left: 266px;"></i></td>
  </tr>
  <tr>
    ...
    border: 1px solid #ccc;
    border-radius: 4px;"
    required placeholder="Enter New Password"pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,10}" 
